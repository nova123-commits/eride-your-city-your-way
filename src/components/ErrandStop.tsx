...
const ErrandStop: React.FC<ErrandStopProps> = ({ stop, onAdd, onRemove }) => {
  ...
  if (stop) {
    return (
      <motion.div
        ...
        className="flex items-center gap-3 p-3 rounded-xl bg-accent border border-primary/20"
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
            ...
            <span>{stop.waitMinutes} min wait · KES {stop.waitMinutes * 5} fee</span>
          </div>
        </div>
        <button onClick={onRemove} className="w-6 h-6 rounded-full bg-destructive/10 flex items-center justify-center">
  ...
  if (!isAdding) {
    return (
      <motion.button
        ...
        className="w-full flex items-center gap-2 p-3 rounded-xl border border-dashed border-border text-muted-foreground text-xs font-medium hover:border-primary/40 transition-colors"
      >
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2">
        <button onClick={() => setIsAdding(false)} className="flex-1 py-2 rounded-lg border border-border text-muted-foreground text-xs font-medium">
          Cancel
        </button>
        <button
          onClick={() => { if (address.length > 2) onAdd({ address, waitMinutes: waitMins }); }}
          disabled={address.length < 3}
          className="flex-1 py-2 rounded-lg brand-gradient text-primary-foreground text-xs font-semibold disabled:opacity-40"
        >
          Add Stop
        </button>
      </div>
